...
- **配置设置**: 打开配置管理窗口
- **历史记录**: 打开历史记录查看窗口
- **任务计划管理**: 安装/卸载/查看 Windows 任务计划（用于开机和唤醒自动触发）
- **禁用监控/启用监控**: 切换监控状态
- **手动触发**: 立即执行一次监控任务
- **退出**: 退出应用（会提示确认）
...
**功能说明**:
任务计划用于在电脑**开机**和从**睡眠唤醒**时自动触发监控。如果不安装任务计划，只有手动触发功能可用。

**主要功能**:
- **查看状态**: 显示任务计划的安装状态
- **安装任务计划**: 一键安装开机和唤醒任务（需要管理员权限）
- **卸载任务计划**: 一键卸载所有任务计划
- **详细信息**: 显示任务计划的具体配置

**安装步骤**:
1. 打开"任务计划管理"
2. 如果未以管理员权限运行，点击"安装任务计划"会提示以管理员身份重新启动 GUI
3. 确认后，系统会以管理员权限重新打开 GUI
4. 再次打开"任务计划管理"，点击"安装任务计划"即可成功安装

**安装后的任务**:
- `DonTouchMe_Boot` - 开机时自动运行（延迟 10 秒）
- `DonTouchMe_Wake` - 从睡眠唤醒时自动运行（延迟 5 秒）
...
### Q4: 睡眠唤醒后没有触发监测怎么办？
A: 需要安装 Windows 任务计划。右键托盘图标 > 任务计划管理 > 安装任务计划。详细说明请查看 `任务计划管理说明.md`。
...
✅ **Stage 4.5**: GUI 任务计划管理 - 已完成（可视化安装/卸载任务计划）
