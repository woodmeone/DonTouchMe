...
echo 3. 下载并安�?Inno Setup 6.x
echo 4. 重新运行本脚本验�?echo.
pause
exit /b 1
...
if exist "%LANG_DIR%" (
    echo.
    echo 可用的语言文件�?    dir /b "%LANG_DIR%*.isl" 2>nul
    echo.

    :: 检测中文语言文件
    if exist "%LANG_DIR%ChineseSimplified.isl" (
        echo �?支持简体中文（ChineseSimplified.isl�?    ) else if exist "%LANG_DIR%ChineseSimp.isl" (
        echo �?支持简体中文（ChineseSimp.isl�?    ) else (
        echo ⚠️  未检测到简体中文语言文件
        echo    使用英文界面（默认）
    )
)
...
echo 检测结�?echo ========================================
echo.
echo �?Inno Setup 已正确安�?echo �?可以开始生成安装包
echo.
echo 下一步操作：
echo 1. 双击运行 build_all.bat
echo 2. 等待生成完成
echo 3. �?installer_output 目录找到安装�?echo.
